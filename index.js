...
const adminStatus = async (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
    ...
    if (req.user.company_name === "AdminCompany") {
      next();
    } else {
      return res.status(404).json({ message: "Account login not Admin" });
    }
  });
};
...
app.post("/submitformNotFile", controllerUser.register_notFile);
app.post(
  "/submitformFile",
  use_uploads_fileRegisterForm.single("file"),
  controllerUser.register_withFile
);
app.get(
  "/getRegisterData",
...
app.get("/ReporFbtAll", controllerFacebook.ReporFbtAll);
app.get("/ReportFbByDate", controllerFacebook.ReportFbByDate);
app.post("/ListFbAdset", controllerFacebook.ListFbAdset);
app.get("/GetFbAdByAdSet", controllerFacebook.GetFbAdByAdSet);
app.get("/GetFbAdByAdSetByDate", controllerFacebook.GetFbAdByAdSetByDate);
app.get("/GetFbActionByAdsName", controllerFacebook.GetFbActionByAdsName);
app.post("/GetFbActionByAdsSet", controllerFacebook.GetFbActionByAdsSet);
app.post("/PostFbInterest", controllerFacebook.PostFbInterest);
app.post("/PostBehaviors", controllerFacebook.PostBehaviors);
//-------Facebook ------end
//
//-------Google ads************** start
app.post(
...
app.post("/googleAdsDataReport", controllerGoogleAds.googleAdsDataReport);
...
app.get("/ProductList", controllerManagement.ProductList);
...
app.delete("/DeleteCompanyId", [adminStatus], controllerUser.DeleteCompanyId);
